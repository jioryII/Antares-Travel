...
function inicializarPagina() {
    try {
        configurarEventos();
        // Cargar estadísticas de manera opcional
...
async function cargarEstadisticas() {
    try {
        ...
        if (!response.ok) {
            console.warn(`Error HTTP ${response.status} al cargar estadísticas`);
            return;
        }
        ...
        if (resultado.success) {
            ...
            console.warn('Error en respuesta de estadísticas:', resultado.message);
        }
    } catch (error) {
        console.warn('Error cargando estadísticas (no crítico):', error.message);
        // No mostrar error al usuario ya que las estadísticas no son críticas
    }
}
...
async function manejarEnvioFormulario(e) {
    ...
    try {
        ...
        if (resultado.success) {
            mostrarExito(tourEditando ? 'Tour actualizado exitosamente' : 'Tour creado exitosamente');
            cerrarModal();
            setTimeout(() => window.location.reload(), 1500);
        } else {
            mostrarError(resultado.message || 'Error desconocido');
        }
...
function mostrarCargando(mostrar) {
    ...
    // Mostrar/ocultar indicador global
...
// Función para exportar tours
async function exportarTours() {
    try {
        // Obtener los filtros actuales
        const formData = new FormData(document.querySelector('.filtros-tours'));
        ...
        const response = await fetch(`../../api/tours.php?accion=exportar&${params}`);
        
        if (!response.ok) {
            throw new Error('Error al exportar tours');
        }
        ...
        mostrarExito('Tours exportados exitosamente');
    } catch (error) {
        console.error('Error al exportar:', error);
        mostrarError('Error al exportar tours');
    } finally {
        mostrarCargando(false);
    }
}
...
async function verEstadisticasDetalladas() {
    try {
        mostrarCargando(true);
        
        const response = await fetch('../../api/tours.php?accion=estadisticas_detalladas');
        ...
        const modalHTML = `
            <div id="modalEstadisticas" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-screen overflow-y-auto">
                    <div class="flex justify-between items-center p-6 border-b">
                        <h2 class="text-xl font-bold text-gray-900">Estadísticas Detalladas</h2>
                        <button onclick="cerrarModalEstadisticas()" class="text-gray-500 hover:text-gray-700">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="bg-blue-50 p-4 rounded-lg">
                                <h3 class="font-semibold text-blue-900">Tours por Estado</h3>
                                <div class="mt-2 space-y-1">
                                    ${data.estadisticas.por_estado?.map(item => 
                                        `<div class="flex justify-between">
                                            <span class="text-sm">${item.estado}:</span>
                                            <span class="font-semibold">${item.cantidad}</span>
                                        </div>`
                                    ).join('') || '<p class="text-sm text-gray-500">No hay datos</p>'}
                                </div>
                            </div>
                            
                            <div class="bg-green-50 p-4 rounded-lg">
                                <h3 class="font-semibold text-green-900">Tours por Región</h3>
                                <div class="mt-2 space-y-1">
                                    ${data.estadisticas.por_region?.map(item => 
                                        `<div class="flex justify-between">
                                            <span class="text-sm">${item.nombre_region}:</span>
                                            <span class="font-semibold">${item.cantidad}</span>
                                        </div>`
                                    ).join('') || '<p class="text-sm text-gray-500">No hay datos</p>'}
                                </div>
                            ...
                            <div class="bg-yellow-50 p-4 rounded-lg">
                                <h3 class="font-semibold text-yellow-900">Precios Promedio</h3>
                                <div class="mt-2 space-y-1">
                                    <div class="flex justify-between">
                                        <span class="text-sm">Promedio General:</span>
                                        <span class="font-semibold">S/ ${data.estadisticas.precio_promedio || '0.00'}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-sm">Precio Mínimo:</span>
                                        <span class="font-semibold">S/ ${data.estadisticas.precio_minimo || '0.00'}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-sm">Precio Máximo:</span>
                                        <span class="font-semibold">S/ ${data.estadisticas.precio_maximo || '0.00'}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Reservas recientes -->
                        ${data.estadisticas.reservas_recientes && data.estadisticas.reservas_recientes.length > 0 ? `
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <h3 class="font-semibold text-gray-900 mb-3">Reservas Recientes</h3>
                            <div class="space-y-2">
                                ${data.estadisticas.reservas_recientes.map(reserva => `
                                    <div class="flex justify-between items-center bg-white p-3 rounded">
                                        <div>
                                            <p class="font-medium">${reserva.nombre_tour}</p>
                                            <p class="text-sm text-gray-600">${reserva.nombre_cliente}</p>
                                        </div>
                                        <div class="text-right">
                                            <p class="font-semibold">S/ ${reserva.total}</p>
                                            <p class="text-sm text-gray-600">${reserva.fecha_reserva}</p>
                                        </div>
                                    </div>
                                `).join('')}
                            </div>
                    ...
                    <div class="flex justify-end p-6 border-t">
                        <button onclick="cerrarModalEstadisticas()" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">
                            Cerrar
                        </button>
                    </div>
                </div>
        ...
        mostrarError('Error al cargar estadísticas detalladas');
    } finally {
        mostrarCargando(false);
    }
}
...
// Funciones públicas para compatibilidad con index.php
function abrirModalCrear() {
    limpiarFormulario();
    document.getElementById('modalTitle').textContent = 'Nuevo Tour';
    document.getElementById('modalTour').classList.remove('hidden');
...
function verEstadisticasDetalladas() {
    mostrarExito('Función de estadísticas detalladas en desarrollo');
}

function verTour(idTour) {
    editarTour(idTour, true); // Reutilizar función de edición en modo solo lectura
}

function editarTour(idTour, soloLectura = false) {
    tourEditando = idTour;
    cargarDatosTour(idTour, soloLectura);
}

function eliminarTour(idTour) {
    if (confirm('¿Estás seguro de que deseas eliminar este tour?')) {
        eliminarTourConfirmado(idTour);
    }
}

// Función para cerrar modales (compatibilidad con index.php)
function cerrarModal() {
    document.getElementById('modalTour').classList.add('hidden');
    limpiarFormulario();
}

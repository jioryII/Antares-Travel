...
try {
    ...
    $errors[] = "Error al cargar idiomas: " . $e->getMessage();
    $todos_idiomas = [];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $connection = getConnection();
            $check_email_sql = "SELECT COUNT(*) FROM guias WHERE email = ?";
            $check_stmt = $connection->prepare($check_email_sql);
            $check_stmt->execute([$email]);
            
            if ($check_stmt->fetchColumn() > 0) {
                $errors[] = "Ya existe un guía registrado con este email";
            }
        } catch (Exception $e) {
    ...
    // Si no hay errores, crear el guía
    if (empty($errors)) {
        try {
            // Iniciar transacción
            ...
            // Insertar el guía
            $insert_sql = "INSERT INTO guias (nombre, apellido, telefono, email, experiencia, estado, foto_url) 
                          VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            $nuevo_id = $connection->lastInsertId();
            
            // Insertar idiomas del guía
            if (!empty($idiomas_seleccionados)) {
                ...
                foreach ($idiomas_seleccionados as $id_idioma) {
                    $insert_idioma_stmt->execute([$nuevo_id, $id_idioma]);
                }
            }
            ...
            // registrarActividad($admin['id_administrador'], 'crear', 'guias', $nuevo_id, 
            //                  "Creó el guía: $nombre $apellido");
            
            $success = "Guía creado exitosamente";
            
            // Redireccionar después de 2 segundos
            header("refresh:2;url=ver.php?id=$nuevo_id");
            
        } catch (Exception $e) {
            // Revertir transacción en caso de error
            $connection->rollback();
            $errors[] = "Error al crear guía: " . $e->getMessage();
        }
    }
}

$page_title = "Crear Nuevo Guía";
?>
                        ...
                        <div>
                            <div class="flex items-center mb-2">
                                ...
                                <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">
                                    <i class="fas fa-user-plus text-blue-600 mr-3"></i>Crear Nuevo Guía
                                </h1>
                            </div>
                            <p class="text-sm lg:text-base text-gray-600">Registra un nuevo guía turístico en la plataforma</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mb-6 bg-green-50 border border-green-200 rounded-lg p-4">
                        <div class="flex">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-green-600 mt-1">Redirigiendo a los detalles del guía...</p>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div class="space-y-6">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select name="estado" id="estado" 
                                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                                <option value="Libre" <?php echo ($_POST['estado'] ?? 'Libre') === 'Libre' ? 'selected' : ''; ?>>
                                                    Libre
                                                </option>
                                                <option value="Ocupado" <?php echo ($_POST['estado'] ?? '') === 'Ocupado' ? 'selected' : ''; ?>>
                                                    Ocupado
                                                </option>
                                            </select>
                                        </div>
                                ...
                                <div>
                                    ...
                                    <div>
                                        ...
                                        <textarea name="experiencia" id="experiencia" rows="4"
                                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                                  placeholder="Describe la experiencia, especialidades, certificaciones, años de trabajo, etc."><?php echo htmlspecialchars($_POST['experiencia'] ?? ''); ?></textarea>
                                ...
                                <div>
                                    ...
                                    <div>
                                            ...
                                            <?php foreach ($todos_idiomas as $idioma): ?>
                                                <label class="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
                                                    <input type="checkbox" 
                                                           ...
                                                           <?php echo in_array($idioma['id_idioma'], $_POST['idiomas'] ?? []) ? 'checked' : ''; ?>
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <span id="idiomas-count" class="text-sm text-blue-600 font-medium">0</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                                    ...
                                    <button type="submit" 
                                        ...
                                        <i class="fas fa-save mr-2"></i>Crear Guía
                                    </button>
                                </div>
                            </div>
                        </form>
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white rounded-lg shadow-lg p-6">
                            <h3 class="text-lg font-semibold text-gray-900 mb-4">
                                <i class="fas fa-info-circle text-blue-600 mr-2"></i>Información de Ayuda
                            </h3>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <ul class="text-sm text-gray-600 space-y-1">
                                        <li>• Nombre y apellido del guía</li>
                                        <li>• Email único de contacto</li>
                                    </ul>
                                </div>
                                
                                <div>
                                    <h4 class="font-medium text-gray-900 mb-2">Estados del Guía</h4>
                                    <ul class="text-sm text-gray-600 space-y-1">
                                        <li>• <span class="font-medium text-green-600">Libre:</span> Disponible para tours</li>
                                        <li>• <span class="font-medium text-yellow-600">Ocupado:</span> No disponible</li>
                                    </ul>
                                </div>
                        ...
                        <div class="bg-white rounded-lg shadow-lg p-6 mt-6">
                            ...
                            <div class="text-center" id="preview">
                                ...
                                <p class="text-sm text-gray-600" id="preview-email">email@ejemplo.com</p>
                                <div class="flex flex-col items-center space-y-2 mt-2">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                        <i class="fas fa-check-circle mr-1"></i>
                                        <span id="preview-estado">Libre</span>
                                    </span>
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                        <i class="fas fa-globe mr-1"></i>
                                        <span id="preview-idiomas">0</span> idiomas
                                    </span>
                                </div>
                            </div>
                        </div>

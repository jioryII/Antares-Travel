...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!empty($datos_oferta['codigo_promocional'])) {
            $check_codigo = "SELECT COUNT(*) FROM ofertas WHERE codigo_promocional = ?";
            $check_stmt = $connection->prepare($check_codigo);
            $check_stmt->execute([$datos_oferta['codigo_promocional']]);
        ...
        // Insertar oferta
        $campos = array_keys($datos_oferta);
        $placeholders = str_repeat('?,', count($campos) - 1) . '?';
        $sql = "INSERT INTO ofertas (" . implode(',', $campos) . ") VALUES ($placeholders)";
        
        $stmt = $connection->prepare($sql);
        $stmt->execute(array_values($datos_oferta));
        
        $oferta_id = $connection->lastInsertId();
        ...
        if ($aplicable_a === 'Tours_Especificos' && !empty($_POST['tours_seleccionados'])) {
            ...
            foreach ($_POST['tours_seleccionados'] as $tour_id) {
                $stmt_tour->execute([$oferta_id, intval($tour_id)]);
            }
        }
        ...
        if ($aplicable_a === 'Usuarios_Especificos' && !empty($_POST['usuarios_seleccionados'])) {
            ...
            foreach ($_POST['usuarios_seleccionados'] as $usuario_id) {
                $stmt_usuario->execute([$oferta_id, intval($usuario_id)]);
            }
        }
        ...
        if (isset($_FILES['imagen_banner']) && $_FILES['imagen_banner']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($extension, $allowed)) {
                $filename = 'oferta_' . $oferta_id . '_' . time() . '.' . $extension;
                ...
                if (move_uploaded_file($_FILES['imagen_banner']['tmp_name'], $filepath)) {
                    ...
                    $stmt_imagen->execute([$imagen_url, $oferta_id]);
                }
            }
        }
        ...
        if (isset($_POST['save_and_continue'])) {
            header("Location: editar.php?id=$oferta_id");
        } else {
...
<body class="bg-gray-50">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        <div>
                            <h1 class="text-xl lg:text-3xl font-bold text-gray-900">
                                <i class="fas fa-plus-circle text-red-600 mr-2 lg:mr-3"></i><?php echo $page_title; ?>
                            </h1>
                            <p class="text-sm lg:text-base text-gray-600 mt-1">Configura una nueva oferta promocional</p>
                        </div>
                ...
                <form method="POST" enctype="multipart/form-data" class="space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="descripcion" name="descripcion" rows="3"
                                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500"
                                          placeholder="Describe los detalles de la oferta..."><?php echo htmlspecialchars($_POST['descripcion'] ?? ''); ?></textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="tipo_oferta" name="tipo_oferta" required
                                    ...
                                    <option value="Porcentaje" <?php echo ($_POST['tipo_oferta'] ?? '') === 'Porcentaje' ? 'selected' : ''; ?>>
                                        Descuento por Porcentaje
                                    </option>
                                    <option value="Monto_Fijo" <?php echo ($_POST['tipo_oferta'] ?? '') === 'Monto_Fijo' ? 'selected' : ''; ?>>
                                        Descuento Monto Fijo
                                    </option>
                                    <option value="Precio_Especial" <?php echo ($_POST['tipo_oferta'] ?? '') === 'Precio_Especial' ? 'selected' : ''; ?>>
                                        Precio Especial
                                    </option>
                                    <option value="2x1" <?php echo ($_POST['tipo_oferta'] ?? '') === '2x1' ? 'selected' : ''; ?>>
                                        2x1 - Paga Uno Lleva Dos
                                    </option>
                                    <option value="Combo" <?php echo ($_POST['tipo_oferta'] ?? '') === 'Combo' ? 'selected' : ''; ?>>
                                        Oferta Combo
                                    </option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="estado" name="estado"
                                    ...
                                    <option value="Activa" <?php echo ($_POST['estado'] ?? '') === 'Activa' ? 'selected' : ''; ?>>
                                        Activa
                                    </option>
                                    <option value="Pausada" <?php echo ($_POST['estado'] ?? '') === 'Pausada' ? 'selected' : ''; ?>>
                                        Pausada
                                    </option>
                                </select>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div id="campo_valor_descuento" class="form-section">
                                ...
                                <div class="relative">
                                    <input type="number" id="valor_descuento" name="valor_descuento" step="0.01" min="0"
                                           ...
                                           placeholder="0.00"
                                           value="<?php echo htmlspecialchars($_POST['valor_descuento'] ?? ''); ?>">
                            ...
                            <div id="campo_precio_especial" class="form-section hidden">
                                ...
                                <input type="number" id="precio_especial" name="precio_especial" step="0.01" min="0"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500"
                                       placeholder="0.00"
                                       value="<?php echo htmlspecialchars($_POST['precio_especial'] ?? ''); ?>">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" id="monto_minimo" name="monto_minimo" step="0.01" min="0"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500"
                                       placeholder="0.00"
                                       value="<?php echo htmlspecialchars($_POST['monto_minimo'] ?? ''); ?>">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" id="limite_usos" name="limite_usos" min="1"
                                       ...
                                       value="<?php echo htmlspecialchars($_POST['limite_usos'] ?? ''); ?>">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" id="limite_por_usuario" name="limite_por_usuario" min="1"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500"
                                       placeholder="1"
                                       value="<?php echo htmlspecialchars($_POST['limite_por_usuario'] ?? '1'); ?>">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <select id="aplicable_a" name="aplicable_a" 
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500">
                                    <option value="Todos" <?php echo ($_POST['aplicable_a'] ?? 'Todos') === 'Todos' ? 'selected' : ''; ?>>
                                        Todos los tours
                                    </option>
                                    <option value="Tours_Especificos" <?php echo ($_POST['aplicable_a'] ?? '') === 'Tours_Especificos' ? 'selected' : ''; ?>>
                                        Tours específicos
                                    </option>
                                    <option value="Usuarios_Especificos" <?php echo ($_POST['aplicable_a'] ?? '') === 'Usuarios_Especificos' ? 'selected' : ''; ?>>
                                        Usuarios específicos
                                    </option>
                                    <option value="Nuevos_Usuarios" <?php echo ($_POST['aplicable_a'] ?? '') === 'Nuevos_Usuarios' ? 'selected' : ''; ?>>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" id="codigo_promocional" name="codigo_promocional"
                                       ...
                                       value="<?php echo htmlspecialchars($_POST['codigo_promocional'] ?? ''); ?>">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" id="mensaje_promocional" name="mensaje_promocional" maxlength="500"
                                       ...
                                       value="<?php echo htmlspecialchars($_POST['mensaje_promocional'] ?? ''); ?>">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="terminos_condiciones" name="terminos_condiciones" rows="4"
                                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500"
                                          placeholder="Especifica los términos y condiciones de la oferta..."><?php echo htmlspecialchars($_POST['terminos_condiciones'] ?? ''); ?></textarea>
                            </div>
                            ...
                            <div class="md:col-span-2 flex flex-wrap gap-4">
                                <div class="flex items-center">
                                    <input type="checkbox" id="visible_publica" name="visible_publica" 
                                           class="rounded border-gray-300 text-red-600 focus:ring-red-500"
                                           <?php echo isset($_POST['visible_publica']) ? 'checked' : 'checked'; ?>>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 justify-end">
                        ...
                        <button type="submit" name="save_and_continue"
                            ...
                            <i class="fas fa-save mr-2"></i>Guardar y Continuar Editando
                        </button>
                        <button type="submit" 
                                class="px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700">
                            <i class="fas fa-check mr-2"></i>Crear Oferta
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        // Generar código promocional automático
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('fecha_inicio').min = fechaMinima;
        });
    </script>
</body>
</html>
